...
test("content-format: input 按字段定语言", () => {
  ...
  assert.equal((formatToolInput("run_python", { code: "print(1)" }) as { lang: string }).lang, "python");
...
test("content-format: output 区分语言与纯文本(grep 判断)", () => {
  ...
  // 自定义工具返回 JSON → 高亮
  assert.equal((formatToolOutput("x", '{"found":true}') as { lang: string }).lang, "json");
  // 非 JSON 文本 → 纯文本
  assert.ok("plain" in formatToolOutput("x", "查不到该数据"));
});
