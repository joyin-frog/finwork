...
// WP4 / AC4.2: 报销制度文件路径可配置;缺省回落内置示例并显式提示"示例制度";
// 配置了贵司制度后不再提示。直接构造并调用 read_expense_policy handler(行为测试,非源码匹配)。
export const expensePolicyConfigTestPromise = (async () => {
  ...
  try {
    ...
    // 默认(未导入贵司制度)→ 回落内置示例 + 显式示例提示
    const def = await readPolicy!({ section: "全部" });
    const defText = def.content.map((c) => c.text).join("");
    assert.ok(defText.includes("示例报销制度"), "AC4.2 FAIL: 缺省应提示当前用的是示例制度");

    // 导入贵司制度后 → 用配置文件内容,且不再提示示例
    ...
    assert.ok(fbText.includes("示例报销制度"), "AC4.2 FAIL: 配置文件不存在时应回落示例并提示");

    console.log("expense-policy-config: all 3 checks passed ✓");
  } finally {
