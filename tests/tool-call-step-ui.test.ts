...
export const toolCallStepUiTestPromise = (async () => {
  ...
  // ── C-D: 密度收敛(第二轮) 源码契约 ──
  {
    ...
    assert.ok(/<details\s+className="[^"]*\bdetails-fold\b[^"]*\btext-body\b/.test(askSrc), "C-D5 FAIL: 已确认摘要行应为 text-body 且使用统一折叠样式");
    // C-E: 呼吸感与连接线(第三轮)
    ...
    assert.ok(cssSrc.includes(".fa-thread") && !cssSrc.includes("border-bottom-left-radius"), "C-E2c FAIL: globals.css fa-thread 应为直线连接(去掉圆角弯钩)");
