...
async function main() {
  ...
  try {
    const mod = await import("../lib/settings/agent-settings");

    // 1. 合法用户名 + 小图头像:落库后读回一致(白名单没漏字段)。
    await mod.writeAgentSettings({ userName: "  Joyin  ", userAvatar: SMALL_PNG });
    const afterValid = await mod.readAgentSettings();
    ...
    await mod.writeAgentSettings({ userAvatar: "javascript:alert(1)" });
    equal((await mod.readAgentSettings()).userAvatar, "", "非 data:image/ 前缀的头像应被丢弃");

    // 3. 超大 data URL(>512KB):丢弃,防止撑爆 settings.json。
    const huge = "data:image/png;base64," + "A".repeat(512 * 1024);
    await mod.writeAgentSettings({ userAvatar: SMALL_PNG }); // 先放一个合法值
    await mod.writeAgentSettings({ userAvatar: huge });
    equal((await mod.readAgentSettings()).userAvatar, "", "超 512KB 的头像应被丢弃");

    // 4. 显式空串:清空头像。
    await mod.writeAgentSettings({ userAvatar: SMALL_PNG });
    await mod.writeAgentSettings({ userAvatar: "" });
    equal((await mod.readAgentSettings()).userAvatar, "", "空串应清空头像");

    // 5. 未传头像(undefined)时保留原值,不误清空。
    await mod.writeAgentSettings({ userAvatar: SMALL_PNG });
    await mod.writeAgentSettings({ userName: "Alice" });
    const preserved = await mod.readAgentSettings();
