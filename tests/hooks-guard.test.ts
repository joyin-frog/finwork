...
(async () => {
  ...
  assert.equal((await h1.before!(py() as never)).action, "allow", "SG FAIL: 4 次报错不该断路");
  // 第 5 次连续报错 → 断路(无 resolver → deny 并导向 AskUserQuestion)
  await err(h1);
  const denied = await h1.before!(py() as never);
  assert.equal(denied.action, "deny", "SG FAIL: 连续 5 次报错应断路");
  ...
  await err(h2); await err(h2); await err(h2); await err(h2); await ok(h2);
  ...
  await err(h3); await err(h3); await err(h3); await err(h3); await err(h3);
  ...
  await err(h4); await err(h4); await err(h4); await err(h4); await err(h4);
