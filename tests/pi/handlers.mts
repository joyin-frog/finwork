...
try {
  ...
  await tool("remember_convention").execute(
    "handler-memory",
  ...
  assert.match(readFileSync(memoryPath, "utf8"), /AR10 handler 约定仅用于隔离测试/);
  ...
  console.log("Pi handlers ✓ read_document path deny, remember_convention and process_voucher_batch used production handlers");
} finally {
