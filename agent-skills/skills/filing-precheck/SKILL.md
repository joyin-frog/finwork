...
starter: "申报前帮我复核一遍"
category: finance
description: 申报前合规复核。报税期用户在对话里发起"申报前帮我复核"时使用：按 A/B/C 三组清单逐项核验义务就绪度、申报数据准确性和遗漏风险，输出按风险排序的三态清单。执行方为主对话（需要画像、工具和问用户通道）。涉及"申报前/复核/报税前检查"即用本技能。
---
...
- 若用户在本次对话中已上传开票汇总文件或台账导出文件 → 据文件内容提示本期开票概况（张数、金额等）。
- 若无文件 → ❓：**"请自查发票台账登记，确认本期开票与收票已全部登记。本产品台账自动核对功能待后续版本提供。"**
