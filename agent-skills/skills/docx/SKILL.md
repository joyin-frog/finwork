...
name: docx
title: Word 文档处理
summary: 创建和编辑 Word 文档,支持目录、页眉页脚、图片等专业格式。
requires: "Word 文档或要写入的内容"
category: file-tool
description: "用于读取、创建或编辑 .docx，包括报告、模板、修订和图片；PDF、表格或纯文本任务不用。输入 Word/内容与版式要求，完成证据是可打开且版式校验通过的 .docx。"
---
