...
用户复审后撤销“侧栏提升到窗口上缘”的排布。以下规则覆盖本文后续与之冲突的历史设计和验收描述：

- Windows 恢复独立 40px 自绘标题栏并横贯全窗，左侧显示正式品牌 Logo 与产品名 `Finwork`，右侧保留三枚窗口按钮。
- 侧栏与主工作区共同位于标题栏下方，两种界面风格都不再把侧栏拉到窗口顶边。
- 侧栏内部顶栏恢复组件原有 `h-11`（44px）节奏；Windows 只移除 macOS 红绿灯净空，不再消费 `--window-chrome-height`。
- macOS、Linux 和 Web 仍不渲染该品牌标题栏，因此既有侧栏与内容几何保持不变。
- 应用级顶部标签栏仍不恢复。这里新增的是 Windows 原生窗口壳层，不承载业务标签或导航。

当前结构：

```text
AppShell（纵排）
├── WindowTitleBar（仅 Windows/Tauri 或开发预览；品牌 + 三件套）
└── app-shell-body（横排）
    ├── AppNav（正常高度侧栏）
    └── app-workspace
        └── main.app-main
...
本文 §1-§10 保留为 2026-07-18 方案的历史决策记录；实施与测试以本节为准。
