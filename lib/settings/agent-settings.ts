...
import { modelConfigFromSettings } from "@/lib/settings/model-config";
...
export type AgentSettings = {
  ...
  userAvatar: string;
  /** Cost-efficient model used by Router and default Agent work. */
  fastModel: string;
  /** Strong model selected explicitly or by a complex nested-task contract. */
...
type SettingsEnvelope = Partial<AgentSettings> & {
  agent?: SettingsEnvelope;
  claude?: SettingsEnvelope;
};
 ...
 * `{claude:{agent:…}}`。第三种曾让 source 停在中间层——那层没有 apiUrl/fastModel 等
...
export async function readAgentSettings(): Promise<AgentSettings> {
  ...
  const modelConfig = modelConfigFromSettings(source) ?? { fastModel: "", reasoningModel: "" };
...
export async function writeAgentSettings(next: Partial<AgentSettings>) {
  ...
  const modelConfig = resolveAtomicModelWrite(next, current);
  const settings: AgentSettings = {
...
/**
 * Atomic two-tier model write. A partial pair is rejected by the API parser.
 */
function resolveAtomicModelWrite(
  next: Partial<AgentSettings>,
  current: AgentSettings,
): Pick<AgentSettings, "fastModel" | "reasoningModel"> {
  const touching = next.fastModel !== undefined || next.reasoningModel !== undefined;
  if (!touching) {
    return {
  ...
  return {
    fastModel: (next.fastModel ?? "").trim(),
    reasoningModel: (next.reasoningModel ?? "").trim(),
  };
}
