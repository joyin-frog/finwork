...
export function createRememberRoleConventionTool(sdk: Sdk) {
  ...
  return sdk.tool(
    ...
    [
      "把用户确认或纠正的、属于某位专员职责域的口径,写入该角色的独立记忆(之后派发该角色时自动遵守;智能体 → 角色 → 记忆 可查看删除)。",
      "只记跨任务复用的规则:计算口径、名单/例外、流程偏好(例:\"实习生按劳务报酬算个税\"\"张三的补贴计入基本工资\")。一次性拍板(\"这次疑点放行\")、数值结果、任务状态不要记。",
      "与全局约定的分工:口径明确属于某专员职责域用本工具;跨角色的全局规矩/偏好用 remember_convention。",
      `roleId 可选值:${roleIds}。`,
      "写入是静默的,无需事先询问;调用后在回复里用一句话告知用户已记住这条口径即可。",
    ].join("\n"),
    {
